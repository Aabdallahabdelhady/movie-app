...
@section('content')
    <div class="movie-info border-b border-gray-800">
        <div class="container mx-auto px-4 py-16 flex flex-col md:flex-row">
            <div class="flex-none">
                <img class="" src="{{ $movieDetails->posterPath }}" alt="Abdullah" class="w-54 md:w-96"
                    style="width: 24rem">
            </div>
            <div class="md:ml-24">
                ...
                <div class="flex flex-wrap items-center text-gray-400 text-sm">
                    ...
                    <span class="ml-1">{{ $movieDetails->voteAverage . '%' }}</span>
                ...
                <div class="mt-12">
                    ...
                    <div class="flex mt-4">
                        @foreach ($movieDetails->crews as $crew)
                            <div class="mr-8">
                                <div>{{ $crew['name'] }}</div>
                ...
                <div x-data="{ isOpen: false }">
                    <div class="mt-12">
                        @if ($movieDetails->youtube_video_id)
                            <button @click="isOpen = true"
                                href="https://youtube.com/watch?v={{ $movieDetails->youtube_video_id }}"
                                ...
                                <div class="modal-body px-8 py-8">
                                    <div class="responsive-container overflow-hidden relative" style="padding-top: 56.25%">
                                        <iframe class="responsive-iframe absolute top-0 left-0 w-full h-full"
                                            src="https://youtube.com/embed/{{ $movieDetails->youtube_video_id }}"
                        ...
                        @foreach ($movieDetails->images as $image)
                            <div class="mt-8">
                                <a @click.prevent="isOpen = true; image = '{{ $image['original'] }}'" href="#">
                                    <img src="{{ $image['path'] }}" alt="{{ $movieDetails->title }}"
                                        class="hover:opacity-75 transition ease-in-out duration-150" />
                                </a>
                            </div>
                        @endforeach
